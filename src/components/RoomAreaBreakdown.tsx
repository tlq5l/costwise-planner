...
const RoomAreaBreakdown = ({ rooms, totalArea }: RoomAreaBreakdownProps) => {
  ...
  const sortedRooms = [...rooms].sort((a, b) => b.dimensions.areaFt - a.dimensions.areaFt);
  
  // Calculate percentages for each room
  const roomsWithPercentage = sortedRooms.map((room) => ({
    ...room,
    percentage: (room.dimensions.areaFt / totalArea) * 100,
  }));

  // Prepare data for pie chart
  const chartData = roomsWithPercentage.map((room) => ({
    name: room.roomType,
    value: room.dimensions.areaFt,
  }));
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-3">
                      ...
                      ></div>
                      <h4 className="font-medium capitalize">{room.roomType}</h4>
                    </div>
                    <span className="font-medium">
                      {formatDimension(room.dimensions.areaFt, "sq.ft")}
                    </span>
                  </div>
                  ...
                  <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                    {formatDimension(room.dimensions.widthFt)} × {formatDimension(room.dimensions.heightFt)}
                  </p>
                </motion.div>
              ))}
            </div>
