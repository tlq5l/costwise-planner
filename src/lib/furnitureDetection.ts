...
import type { FurnitureDetectionResponse, FurnitureItem, FurnitureType } from "@/types";
...
function mapToFurnitureType(className: string): FurnitureType {
  ...
  if (lowerClassName.includes('toilet') || lowerClassName.includes('wc')) return FurnitureType.TOILET;
 ...
 */
export async function detectFurnitureWithAnimation(imageBase64: string): Promise<{
 ...
 */
export function countFurnitureByType(furniture: FurnitureItem[]): Record<FurnitureType, number> {
 ...
 */
export function countFurnitureByRoom(furniture: FurnitureItem[]): Record<string, Record<FurnitureType, number>> {
  return furniture.reduce((acc, item) => {
    if (item.room) {
      if (!acc[item.room]) {
        acc[item.room] = {};
