...
import { Project, RoomAnalysisResult } from '@/types';
...
export const projectsApi = {
  ...
  async getProjects(): Promise<Project[]> {
    try {
      ...
      const storedProjects = localStorage.getItem('floor_plan_analyzer_projects');
      if (storedProjects) {
  ...
  async createProject(projectData: Omit<Project, 'id' | 'createdAt' | 'updatedAt' | 'analyses'>): Promise<Project> {
    try {
      ...
      localStorage.setItem('floor_plan_analyzer_projects', JSON.stringify(updatedProjects));
  ...
  async addAnalysisToProject(projectId: string, analysis: RoomAnalysisResult): Promise<Project> {
    try {
      // For now, update local storage
      const storedProjects = localStorage.getItem('floor_plan_analyzer_projects');
      if (storedProjects) {
        ...
        if (updatedProject) {
          return {
            ...
            updatedAt: new Date(),
            analyses: updatedProject.analyses.map((a: any) => ({
              ...a,
...
export const analysesApi = {
  ...
  async processFloorPlan(file: File, projectId?: string): Promise<RoomAnalysisResult> {
    try {
